  ...
  return (
    <AppShell pageTitle="Transaction History" pageSubtitle="Universal Activity Ledger">
      <div className="max-w-7xl mx-auto space-y-12 pb-24">
        ...
        {/* Filter Navigation */}
        <div className="flex gap-4 p-1 bg-[#111827] rounded-[10px] w-fit border border-[#1E2A3A]">
           {['All Activity', 'Lending', 'Borrowing', 'Reputation'].map((t, i) => (
             <button key={i} className={`px-6 py-3 rounded-[8px] text-[10px] font-black uppercase tracking-widest transition-all ${i === 0 ? 'bg-[#F5A623] text-black' : 'text-[#8C8C8C] hover:text-[#FAFAF8]'}`}>
                {t}
             </button>
           ))}
        </div>
        ...
        <div className="bg-[#111827] rounded-[12px] border border-[#1E2A3A] overflow-hidden">
           <div className="overflow-x-auto">
              <table className="w-full text-left">
                 <thead>
                    <tr className="border-b border-[#1E2A3A] bg-[#0A0F1E]/50">
                 ...
                 <tbody className="divide-y divide-[#1E2A3A]">
                    {[
                      ...
                      <tr key={i} className="hover:bg-[#0A0F1E] transition-all group">
                         <td className="px-8 py-8">
                            <div className="flex items-center gap-4">
                               <div className="w-10 h-10 rounded bg-[#0A0F1E] border border-[#1E2A3A] flex items-center justify-center text-[#F5A623]">
                                  <iconify-icon icon={tx.icon} className="text-xl"></iconify-icon>
                               </div>
                               <span className="text-sm font-black text-[#FAFAF8]">{tx.type}</span>
                            </div>
                         </td>
                         <td className="px-8 py-8">
                         ...
                         <td className="px-8 py-8">
                            <span className={`text-sm font-black ${tx.positive ? 'text-[#1D9E75]' : 'text-[#FAFAF8]'}`}>{tx.val}</span>
                         </td>
                         <td className="px-8 py-8">
                         ...
                         <td className="px-8 py-8 text-right">
                            <p className="text-[10px] font-black text-[#FAFAF8] uppercase tracking-widest">{tx.date}</p>
