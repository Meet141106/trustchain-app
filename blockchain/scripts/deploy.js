...
async function main() {
  ...
  // 7. Write contracts.json — SINGLE SOURCE OF TRUTH for all contract addresses
  const contractsJson = {
  ...
  // Write to src/config/contracts.json (primary)
  ...
  console.log("✓ contracts.json written to src/config/.");

  // Write to src/lib/contracts.json (legacy compat)
  ...
  console.log("✓ contracts.json written to src/lib/ (legacy).");
  ...
  // a2) Transfer TRUST to borrower so they can cover interest on repayments
  console.log("  → Transferring 500 TRUST to borrower", borrower.address);
  await trustToken.transfer(borrower.address, hre.ethers.parseEther("500"));
